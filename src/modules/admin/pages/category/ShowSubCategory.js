...
import { apiSubCategoryDelete } from '../../api/ApiList';
import { subCategoryFetch } from '../../../frontend/frontend_api/Axios';
...
export const ShowSubCategory = () => {
...

        {
            ...
            content: (item, path) => (
                <td style={{ display: 'inline-block', width: '90px' }}>
                    <Link to="/admin/subcategory/edit" state={item[path]} className='btn btn-success btn-sm px-1 py-0 me-2'>
                        <AiOutlineEdit />
                    </Link>
                    <button type="button" value={item[path]} onClick={deleteSubCategory} className='btn btn-danger  btn-sm px-1 py-0'>
                        <MdDeleteForever />
                    </button>
                </td>
            )
    ...
    const deleteSubCategory = (e) => {
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    deleteRecord(apiSubCategoryDelete, id)
                        .then((response) => {
