            ...
            <Row>
                <Col md={6}>
                    <div className='d-flex align-items-center' style={{ width: '100%', height: '100%', padding: '20px 10px' }}>
                        <div>
                            ...
                            <h6 style={{ color: 'var(--primaryColor)' }}>{moment(location.state.datetime).format('MMMM Do YYYY, h:mm:ss a')}</h6>
                ...
                <Col md={6}>
                    <div className='d-flex justify-content-center align-items-center product-details-img' style={{ width: '100%', height: '100%', position: 'relative' }}>
                        <img src={"http://localhost:8000/storage/news/" + location.state.image} alt='image' style={{ width: '400px', height: 'auto', position: 'absolute' }} />
                    </div>
                </Col>
            </Row>
            <button type='button' className='btn' style={{ background: 'var(--primaryColor)', color: 'white' }} onClick={() => navigate('/news')}>Back</button>
        </Container>
    )
}
